...
public class WalkOnWater {
	...
	// public static ConcurrentHashMap<Block, Byte> affectedblocks = new
	// ConcurrentHashMap<Block, Byte>();
	...
	// // if (Tools.canBend(player, Abilities.WalkOnWater)) {
	// // players.add(player);
	// // for (Block block : Tools.getBlocksAroundPoint(player.getLocation(),
	// // radius)) {
	// // double dy = player.getLocation().getY() - block.getY();
	...
	// +
	// "Anytime the user walks over water, it is instead turned into a patch of ice. "
	// +
	// "The patch will only remain as long as the user is within a short range or has "
